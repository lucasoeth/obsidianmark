(async function () {
  options = window.obsidianMarkOptions

  let tags = options.defaultTags;
  ...
  function getFileName(fileName) {
    ...
    // TODO: Add more characters using options
  ...
  if (options.vaultName) {
    var vaultName = "&vault=" + encodeURIComponent(options.vaultName);
  } else {
    var vaultName = "";
  }
  ...
  try {
    ...
    chrome.runtime.sendMessage({
      type: "obsidianmark_open_obsidian",
      url:
        "obsidian://advanced-uri" +
        "?clipboard=true&mode=new&filepath=" +
        encodeURIComponent(options.vaultFolder + fileName) +
        vaultName,
    });
  } catch (err) {
